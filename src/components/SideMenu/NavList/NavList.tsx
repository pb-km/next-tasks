...
const NavList = () => {
  const navList: NavItemType[] = [
    ...
    { id: 1, label: 'Expired Tasks', link: '/expired', icon: <FaRegClock className="size-5" />}
    
  ];
  return (
    <div className="mt-24">
        {navList.map((item) => (<NavItem key={item.id} label={item.label} link={item.link} icon={item.icon}></NavItem> ))}
    </div>
  );
};
